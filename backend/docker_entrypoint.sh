...
# export CONFIG_TYPE=config.ProdConfig
...
# sed -i '' "s/{{REGION_NAME}}/$REGION_NAME/g" .env

/bin/sed -i "s|{{DYNAMODB_URI}}|$DYNAMODB_URI|g" .env
# sed -i '' "s|{{DYNAMODB_URI}}|$DYNAMODB_URI|g" .env

/bin/sed -i "s/{{CONFIG_TYPE}}/$CONFIG_TYPE/g" .env
# sed -i '' "s/{{CONFIG_TYPE}}/$CONFIG_TYPE/g" .env

/bin/sed -i "s|{{FLASK_APP}}|$FLASK_APP|g" .env
# sed -i '' "s|{{FLASK_APP}}|$FLASK_APP|g" .env
